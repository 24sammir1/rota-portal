          ...
          <div style={styles.card}>
            ...
            {previewData && (
              <div style={styles.preview}>
                ...
                <p style={styles.previewItem}><span style={styles.previewLabel}>Week Ending:</span> {previewData.weekEnding}</p>
                <p style={styles.previewItem}><span style={styles.previewLabel}>Sheet Name:</span> {previewData.sheetName}</p>
                <p style={styles.previewItem}><span style={styles.previewLabel}>Type:</span> {previewData.type}</p>
                <p style={styles.previewItem}><span style={styles.previewLabel}>Staff Count:</span> {previewData.staff?.length || 0}</p>
              </div>
            )}
            {uploadStatus && (
              <div style={{...styles.alert, ...(uploadStatus.type === 'success' ? styles.alertSuccess : styles.alertError)}}>
            ...
            <button
              ...
              style={{...styles.btn, ...styles.btnPrimary, ...(!previewData || isUploading ? styles.btnDisabled : {})}}
            >
                    ...
                    {rotas.map((rota, idx) => (
                      <tr key={rota.id}>
                        <td style={styles.td}>
                          ...
                          {idx === 0 && <span style={{...styles.statusBadge, ...styles.statusApproved, marginLeft: '0.5rem'}}>Current</span>}
                        </td>
                        <td style={styles.td}>
                          <span style={{...styles.statusBadge, ...(rota.type === 'actual' ? styles.statusApproved : styles.statusPending)}}>
                            {rota.type}
                          </span>
                        </td>
                ...
                {pendingUsers.map((user) => (
                  <div key={user.id} style={styles.userRow}>
                    {editingUser?.id === user.id ? (
                      <div style={{width: '100%'}}>
                        <label style={{color: '#a0a0a0', fontSize: '0.85rem', display: 'block', marginBottom: '0.5rem'}}>
                  ...
                  <tbody>
                    {allUsers.map((user) => (
                      <tr key={user.id}>
                        ...
                        <td style={styles.td}>
                          <span style={{...styles.roleBadge, ...(user.role === 'admin' ? styles.roleAdmin : styles.roleStaff)}}>
                            {user.role}
                          </span>
                        </td>
                        <td style={styles.td}>
                          <span style={{...styles.statusBadge, ...(user.status === 'approved' ? styles.statusApproved : styles.statusDeactivated)}}>
                            {user.status}
                          </span>
                        </td>
                        <td style={{...styles.td, textAlign: 'right'}}>
                            ...
                            <div style={{display: 'flex', gap: '0.5rem', justifyContent: 'flex-end'}}>
                              ...
                              {user.role !== 'admin' && (
                                <button
                                  ...
                                  style={{...styles.btn, ...(user.status === 'approved' ? styles.btnDanger : styles.btnSuccess), ...styles.btnSmall}}
                                >
